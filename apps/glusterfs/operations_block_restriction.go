...
func (ro *VolumeSetBlockRestrictionOperation) checkCanUnlock(
	...
	switch {
	case v.Info.BlockInfo.Restriction == r:
		// no change is always OK
		return nil
	case r != api.Unrestricted:
		// locking down the volume is OK
		return nil
	case v.Info.BlockInfo.Restriction == api.Locked:
		// unlocking an admin locked volume is OK
		return nil
	case v.Info.BlockInfo.Restriction == api.LockedByUpdate:
